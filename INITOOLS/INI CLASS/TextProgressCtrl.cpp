...
void CTextProgressCtrl::OnPaint() 
{
    ...
    if (m_bShowText)
    {
		...
		{
            //str = m_strText;
			str.Format("%s %d%%",m_strText.GetBuffer(0),(int)(Fraction*100.0));
		}
        else if(Fraction>0&&m_strText.IsEmpty())
            str.Format("%d%%", (int)(Fraction*100.0));
		else if(Fraction==0&&m_strText.GetLength())
			str=m_strText;
...
void CTextProgressCtrl::SetText(CString str)
{
	m_strText=str;
	RedrawWindow();
} 